...
export default function ComingSoonBanner({ variant = "home", className = "" }: ComingSoonBannerProps) {
  const isHomePage = variant === "home";
  
  return (
    <div className={`backdrop-blur-md rounded-xl shadow-2xl p-8 ${isHomePage ? 'bg-white/10' : 'bg-white/95'} ${className}`}>
      <div className="text-center">
        <div className="mb-6">
          <Calendar className={`w-12 h-12 mx-auto mb-4 ${isHomePage ? 'text-accent' : 'text-primary'}`} />
          <h2 className={`text-3xl md:text-4xl font-bold mb-4 ${isHomePage ? 'text-white' : 'text-primary'}`}>
            Скоро открытие!
          </h2>
          <p className={`text-lg md:text-xl mb-2 ${isHomePage ? 'text-gray-100' : 'text-gray-700'}`}>
            Наши роскошные гостевые дома откроются
          </p>
          <p className={`text-xl md:text-2xl font-semibold ${isHomePage ? 'text-accent' : 'text-primary'}`}>
            после 20 августа 2025
          </p>
        </div>
        
        <div className="mb-8">
          <p className={`text-base md:text-lg mb-4 ${isHomePage ? 'text-gray-200' : 'text-gray-600'}`}>
            Узнайте первыми о нашем открытии и специальных предложениях
          </p>
          <p className={`text-sm md:text-base ${isHomePage ? 'text-gray-300' : 'text-gray-500'}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Link href="/about">
            <Button 
              ...
              className={`${
                isHomePage 
                  ? 'border-white text-white hover:bg-white/20 hover:border-accent hover:text-accent' 
